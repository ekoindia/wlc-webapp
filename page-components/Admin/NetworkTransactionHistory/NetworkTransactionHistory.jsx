...
import { HistoryTable, HistoryToolbar } from "page-components";
...
/**
 * A NetworkTransactionHistory component shows transaction history
 * @param 	{object}	prop	Properties passed to the component
 * @param	{string}	prop.prop1	TODO: Property description.
 * @param	{...*}	rest	Rest of the props passed to this component.
 * @example	`<NetworkTransactionHistory></NetworkTransactionHistory>` TODO: Fix example
 */
const NetworkTransactionHistory = () => {
	const formElements = {
		tid: "",
		account: "",
		customer_mobile: "",
	...
	const hitQuery = (abortController, key) => {
		console.log("[NetworkTransactionHistory] fetch started...", key);
		...
		fetcher(process.env.NEXT_PUBLIC_API_BASE_URL + Endpoints.TRANSACTION, {
			...
			.then((data) => {
				...
				setData(tx_list);
				console.log(
					"[NetworkTransactionHistory] fetch result...",
					key,
					tx_list
				);
			})
			.catch((err) => {
				console.error("[NetworkTransactionHistory] error: ", err);
			})
			.finally(() => {
				setLoading(false);
			});
	...
	const clearFilter = () => {
		...
		router.push(`${prefix}/network-transaction-history`, undefined, {
			shallow: true,
		});
	};
	...
	const onSearchSubmit = ({ search }) => {
		...
		if (_validSearch) {
			...
			const prefix = isAdmin ? "/admin" : "";
			router.push(
				`${prefix}/network-transaction-history?search=${search}`,
				undefined,
				{
					shallow: true,
				}
	...
	useEffect(() => {
		console.log(
			"[NetworkTransactionHistory] fetch init",
			currentPage,
			finalFormState
		);
		...
		return () => {
			console.log(
				"[NetworkTransactionHistory] fetch aborted... ",
				currentPage,
	...
	return (
		<>
			<Headings title="Network Transaction History" />
			<Flex
				w="full"
				h="auto"
...
export default NetworkTransactionHistory;
