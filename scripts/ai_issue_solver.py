...
system_prompt = """You are an expert software engineer specializing in JavaScript/TypeScript/React.
...
user_prompt = (
    ...
    f"### Issue Details\n{ISSUE_BODY}\n\n"
    f"### Parsed Error\n"
    f"- **Error**: {issue_details['error']}\n"
    f"- **Culprit**: {issue_details['culprit']}\n"
    f"- **Stack Trace**:\n{issue_details['stack_trace'][:2000]}\n\n"
    f"### Relevant Source Files\n{file_context}\n\n"
    f"### Build Log\n{BUILD_LOG[:3000] if BUILD_LOG else 'No build errors.'}\n\n"
    "Return ONLY a valid JSON object with the minimal fix."
)
...
try:
    # strict=False accepts literal control chars (newlines/tabs) inside JSON strings
...
fix_data["files"] = safe_files[:MAX_CHANGED_FILES]

# ── Write ai_file_changes.json ────────────────────────────────────────────────

with open("ai_file_changes.json", "w", encoding="utf-8") as out:
    json.dump(fix_data, out, indent=2)

print(f"✅ Written ai_file_changes.json ({len(fix_data['files'])} file(s))")
...
changed_paths = [f["path"] for f in fix_data["files"]]
changed_list = "\n".join(f"- `{p}`" for p in changed_paths) or "No files changed"
...
pr_description = (
    ...
    f"- AI Model: `{used_model}`\n\n"
    "> ⚠️ This PR was auto-generated by AI. Please review carefully before merging.\n"
)

with open("ai_pr_description.md", "w", encoding="utf-8") as out:
    out.write(pr_description)

print("✅ Written ai_pr_description.md")
print(f"🎉 Done! Fix generated for {len(fix_data['files'])} file(s).")